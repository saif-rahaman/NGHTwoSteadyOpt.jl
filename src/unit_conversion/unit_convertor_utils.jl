...
function _get_data_units(rescale_functions)::Dict{Symbol,Any}
    ...
    initial_node_injection_units = Dict{String,Function}(
        "node_slack_injection" => rescale_pressure
    )

    initial_node_pressure_units = Dict{String,Function}(
        "node_pressure" => rescale_pressure
    )
    ...
    units[:initial_node_injection_units] = initial_node_injection_units
...
function _rescale_data!(data::Dict{String,Any}, 
    ...
    initial_node_injection_units = units[:initial_node_injection_units]
    ...
    for (param, f) in merge(initial_node_injection_units, initial_node_pressure_units, initial_pipe_flow_units, initial_compressor_flow_units)
        for (i, value) in get(data, param, [])
            data[param][i] = f(value)
        end 
    end 

    for (param, f) in boundary_flow_units 
        for (i, value) in get(data, param, [])
            injection = value["injection"]
            withdrawal = value["withdrawal"]
            (~isnothing(injection)) && (data[param][i]["injection"] = f(injection))
            (~isnothing(withdrawal)) && (data[param][i]["withdrawal"] = f(withdrawal))
        end 
    end 

    for (param, f) in boundary_pressure_units
        for (i, value) in get(data, param, [])
            data[param][i]["pressure"] = f(value["pressure"])          
        end 
    end 
